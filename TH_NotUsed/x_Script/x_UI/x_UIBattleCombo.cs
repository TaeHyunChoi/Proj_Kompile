...
public class x_UIBattleCombo : MonoBehaviour
{
    public static x_UIBattleCombo Instance { get => instance; }
    private static x_UIBattleCombo instance;
    ...
    public static void Init()
    {
        ...
        go = Instantiate(go, x_UIMgr.Canvas_Battle.transform);
        instance = go.GetComponent<x_UIBattleCombo>();
    ...
    public static void Show(bool isOn)
    {
        ...
        x_UnitMgr.Render_SetOrder(isTurn: true);
    }
    private void InitSlotIcons()
    {
        x_Unit actor = x_UnitMgr.InBattle[x_GameMgr.NowOrder];
        int index = actor.Data.Index;
        byte[] combo = x_Player.MemberCombo[index];

        SkillData skill;
        center = x_CameraMgr.Battle_ScreenToLocalInRect(actor.Pos);
        ...
        for (int i = 0; i < combo.Length; ++i)
        {
            ...
            skill = x_DataMgr.SkillTBL[combo[i]];
            iconSlots[i].sprite = x_ResourceMgr.SPIcon[skill.RscCode];
    ...
    public bool UpdateUI(bool active, float lerpWeight)
    {
        ...
        //dim alpha 
        color = dimBlack.color;
        dimBlack.color = new Color(color.r, color.g, color.b, Mathf.Lerp(color.a, alphaEnd * 0.75f, x_IDxVALUE.LERP * lerpWeight));

        //slot position, alpha
        for (int i = 0; i < rectSlots.Length; ++i)
        {
            positionEnd = active ? endPos[i] : center;
            rectSlots[i].localPosition = Vector3.Lerp(rectSlots[i].localPosition, positionEnd, x_IDxVALUE.LERP * lerpWeight);

            color = iconSlots[i].color;
            iconSlots[i].color = new Color(color.r, color.g, color.b, Mathf.Lerp(color.a, alphaEnd, x_IDxVALUE.LERP * lerpWeight));
        }
