...
public class x_UnitCoroutine : MonoBehaviour
{
    private x_Unit owner;
    ...
    private void InitBase(int type)
    {
        ...
        owner = transform.GetComponent<x_Unit>();
    ...
    private void SetNext_Attack()
    {
        switch (state[curType])
        {
            case 0:
                ...
                break;
            case 1:
                owner.Anime_Play(x_IDxUNIT.ANIME_SKILL);
                break;
            case 2:
                owner.Anime_Play(x_IDxUNIT.ANIME_IDLE);
                if (owner != x_UnitMgr.MyPC)
                    break;
                
                x_UnitMgr.Anime_PlaySlow(slow: false, 1.1f);
                break;
            case 3:
    ...
    private bool MoveNext_Attack()
    {
        switch (state[curType])
        {
            ...
            case 1:
                ...
                if (!state.IsName(x_IDxUNIT.ANIME_SKILL))
                    return false;

                return state.normalizedTime >= 1;
            case 2:
                return x_UIMgr.Battle_UpdateUICombo(false, 1.1f);
            case 3:
                return endTime <= Time.time;
        }
    ...
    public void InitHit(x_Unit hitter, SkillData hitSkill)
    {
        InitBase(type: 1);
    ...
    private void SetNext_Hit()
    {
        ...
        {
            case 0:
                owner.Status[x_IDxUNIT.HP] -= (int)fValue;
                owner.Anime_Play(x_IDxUNIT.ANIME_HIT);
                endTime = Time.time + owner.Anime_GetLength(x_IDxUNIT.ANIME_HIT);
                break;
            default:
                owner.Anime_Play(x_IDxUNIT.ANIME_IDLE);
                enabled = false;
