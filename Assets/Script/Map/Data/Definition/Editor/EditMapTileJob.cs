...
namespace Script.Data
{
    ...
    public struct EditMapTileJob : IJobParallelFor
    {
        ...
        public void Execute(int index)
        {
            ...
            {
                ID          = MapCoordUtil.ComputeTileID(rotatedPivot),
                NaviMask    = naviMask,
                LinkMask    = default,
        ...
        {
            ulong layerMask = (ulong)navLayer << (MapConsts.TOTAL_BITS * MapConsts.BITS_PER_CELL);
            ...
            {
                int size = MapConsts.MATRIX_SIZE;
                ulong cellValue;

                ulong[,] matrix = new ulong[size, size];
                int x, y;
                for (int i = 0; i < MapConsts.TOTAL_BITS; ++i)
                {
                    cellValue = mask & Map.Data.MapConsts.HEIGHT_MASK;
                    x = MapConsts.INDEX_MAP[i].x;
                    y = MapConsts.INDEX_MAP[i].y;
                    matrix[x, y] = cellValue;

                    mask >>= MapConsts.BITS_PER_CELL;
                }
            ...
            ulong MatrixToBitmask(ulong[,] matrix)
            {
                ...
                for (int i = 0; i < MapConsts.TOTAL_BITS; ++i)
                {
                    ...
                    newMask |= mask << i * MapConsts.BITS_PER_CELL;
                }
