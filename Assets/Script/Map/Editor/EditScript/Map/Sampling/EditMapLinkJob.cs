...
/// <summary>
/// [MapSampling] 주변 타일과의 연결 여부를 확인 및 저장한다.
/// </summary>
[BurstCompile]
public struct EditMapLinkJob : IJobParallelFor
{
    ...
    public void Execute(int index)
    {
        ...
        for (int i = 0; i < LinkDirs.Length; i++)
        {
            ...
            EditMapTileDirFlag dirFlag = EditMapUtil.GetDirFlag(dir2D.x, dir2D.y);

            for (int y = 0; y < DiffYs.Length; y++)
            {
                float dy = DiffYs[y];
                float3 targetDir = new float3(dir2D.x, dy, dir2D.y);
                long neighborID = MapCoordUtil.ComputeTileID(myPivot + targetDir);
                ...
                if (true == Map.TryGetValue(neighborID, out EditMapTileData neighborTile))
                {
                    if (true == CheckConnectable(myTile, neighborTile, targetDir, dirFlag))
                    {
                        ...
                        accumulatedLinkMask |= (linkValue << MapCoordUtil.GetLinkMaskShift(dirFlag));
                        break;
                    }
                }
            }
    ...
    private readonly bool CheckConnectable(EditMapTileData myTile, EditMapTileData neighborTile, float3 dir, EditMapTileDirFlag dirFlag)
    {
        // 직선 방향 체크
