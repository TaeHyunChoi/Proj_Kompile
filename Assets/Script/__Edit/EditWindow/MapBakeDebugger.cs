...
namespace Kompile.Map.Editor.Tools
{
    ...
    public class MapBakeDebugger : EditorWindow
    {
        ...
        private void DrawGridDetails(MapGridData grid)
        {
            ...
            EditorGUILayout.LabelField($"🟩 Tile Logical Data (Total: {grid.NaviTileDict?.Count ?? 0})",
                EditorStyles.boldLabel);
            EditorGUILayout.HelpBox(
                "Pivot: Grid/Tile Key를 변환한 월드 좌표\nNavi: 13개 정점의 높이 배열 (-1 ~ 8)\nLink: 8방향 이웃 타일과의 Y 단차 (0, 1, -1)",
                MessageType.Info);
            ...
            foreach (var kvp in grid.NaviTileDict)
            {
                ...
                try
                {
                    ...
                    InUtilMapKey.GetPivot(grid.Key, tileKey, out float3 pivot);
