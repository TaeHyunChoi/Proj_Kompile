    ...
    public static class InUtilMapGeometry
    {
        ...
        public static bool IsTilePointWalkable(in MapTileData tile, in float2 localPos)
        {
            for (int s = 0; s < MapConsts.TRIANGLES_COUNT; s++)
            {
                ...
                {
                    return InUtilMapNavi.IsSubTileValid(tile.NaviMask, s);
                }
            }
            return false;
        }
        ...
        public static bool TrySampleTileHeight(in MapTileData tile, in float tileBaseY, in float2 localPos, in float heightStep, out float groundY)
        {
            ...
            for (int s = 0; s < MapConsts.TRIANGLES_COUNT; s++)
            {
                if (!InUtilMapNavi.IsSubTileValid(tile.NaviMask, s)) 
                    continue;                    
                ...
                int h0 = InUtilMapNavi.GetHeightFromNaviMask(tile.NaviMask, v0);
                int h1 = InUtilMapNavi.GetHeightFromNaviMask(tile.NaviMask, v1);
                int h2 = InUtilMapNavi.GetHeightFromNaviMask(tile.NaviMask, v2);
