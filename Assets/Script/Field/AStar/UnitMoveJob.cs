namespace Kompile.Data
{
    ...
    public struct UnitMoveJob : IJobParallelFor
    {
        ...
        private bool TrySampleHeight(float3 pos, out float groundY)
        {
            ...
            return InUtilMapGeometry.TrySampleTileHeight(in tileInfo.TileData, in tileInfo.TileBaseY, in localPos, HEIGHT_STEP, out groundY);
        }
        ...
        private bool TryGetTileAtBurst(float3 curPos, int targetTx, int targetTz, float referenceY, out TileInfoContext tileInfo)
        {
            ...
            if (dirIndex != -1)
            {
                if (InUtilMapNavi.TryGetYInt(curTileInfo.TileData.LinkMask, dirIndex, out int yOffset))
                {
