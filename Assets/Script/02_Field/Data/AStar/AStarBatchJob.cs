...
[BurstCompile]
    public struct AStarBatchJob : IJobParallelFor
    {
        ...
        public void Execute(int index)
        {
            ...
            long startID = InUtilMapKey.ComputeTileIDInt(startVerticeInt);
            ...
            while (openHeap.Length > 0)
            {
                ...
                long currentID = InUtilMapKey.ComputeTileIDInt(currentNode.VerticeInt);
                ...
                for (int i = 0; i < NEIGHBOR_OFFSETS_INT.Length; i++)
                {
                    ...
                    long targetID = InUtilMapKey.ComputeTileIDInt(targetVerticeInt);
                    ...
                    if (targetID != currentID)
                    {
                        ...
                        targetID = InUtilMapKey.ComputeTileIDInt(targetVerticeInt);
                        if (!Map.TryGetValue(targetID, out targetItem)) continue;
                    }

                    InUtilMapKey.ComputeWorldPositionInt(targetID, out int3 targetPivotInt);
                    targetPivotInt *= PATH_SEARCH_RECIPROCAL;
                    int vIndex = InUtilMapNavi.GetVertexIndexFromLocalPos(targetVerticeInt.x - targetPivotInt.x, targetVerticeInt.z - targetPivotInt.z);
                    
                    if (vIndex != -1)
                    {
                        int heightY = InUtilMapNavi.GetHeightFromNaviMask(targetItem.navi, vIndex);
                    ...
                    InUtilMapKey.ComputeWorldPosition(targetID, out float3 targetPivot);
        ...
        private bool HasLineOfSight(float3 start, float3 end)
        {
            ...
            while (traveled < dist)
            {
                ...
                long id = InUtilMapKey.ComputeTileIDInt(checkInt);

                if (!Map.TryGetValue(id, out var item)) return false;

                InUtilMapKey.ComputeWorldPosition(id, out float3 pivot);
                ...
                int vIndex = InUtilMapNavi.GetVertexIndexFromLocalPos(checkInt.x - pivotInt.x, checkInt.z - pivotInt.z);
                
                if (vIndex != -1)
                {
                    int heightY = InUtilMapNavi.GetHeightFromNaviMask(item.Navi, vIndex);
        ...
        private bool IsVerticeMovable(long naviMask, float3 tilePivot, int3 vertInt, float radius) {
            ...
            for (int s = 0; s < 16; s++) {
                if (InUtilMapNavi.IsCircleOverlappingSubTile(s, localCenter, rSq) && !InUtilMapNavi.IsSubTileValid(naviMask, s)) return false;
            }
            return true;
        }
