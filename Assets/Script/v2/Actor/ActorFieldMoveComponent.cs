namespace Kompile
{
    ...
    public class ActorFieldMoveComponent
    {
        ...
        private bool TrySampleHeight(float3 pos, out float groudY)
        {
            ...
            if (!TryGetTile(pos, tx, tz, pos.y, out BurstTileInfo tileInfo))
            {
                return false;
            }
        ...
        private bool IsPointWalkable(float3 referencePos, float3 point)
        {
            ...
            if (!TryGetTile(referencePos, tx, tz, referencePos.y, out BurstTileInfo tileInfo))
            {
                return false;
            }
        ...
        private bool TryGetTile(float3 curPos, int targetTx, int targetTz, float referenceY, out BurstTileInfo tileInfo)
        {
            ...
            if (-1 != dirIndex
                && MapNaviTileUtil.TryGetYInt(curTileInfo.TileData.LinkMask, dirIndex, out int yOffset))
            {
