...
namespace Kompile.Editor.Provider
{
    ...
    public partial class EditMapSamplingProvider // main
    {
        ...
        public void Bake()
        {
            ...
            {
                InUtilMapKey.ComputeKey(nativeResult[i].ID, out int gridKey, out int tileKey);
            ...
            foreach (KeyValuePair<int, EditMapGridData> grid in map)
            {
                ...
                EditAssetProvider.WriteBinaryFile<MapGridData>(
                    data: mapGridData,
            ...
            EditAssetProvider.WriteBinaryFile<MapRegistryData>(
                data: registryData,
                relativePath: MAP_NAVI_DATA_PATH,
        ...
        private void LinkTiles(ConcurrentDictionary<int, EditMapGridData> map)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                InUtilMapKey.ComputeKey(resultTile.ID, out int gKey, out int tKey);
