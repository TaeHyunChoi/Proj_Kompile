...
namespace Kompile.Map.Editor.Tools
{
    ...
    public class KompileMapEditorWindow : EditorWindow
    {
        ...
        private static readonly Vector2[] PointOffsets = new Vector2[]
        {
            ...
            new Vector2(0.25f, 0.75f), new Vector2(0.75f, 0.75f), new Vector2(0.0f, 1.0f), new Vector2(0.5f, 1.0f),
        ...
        private void LoadAllAtlases()
        {
            ...
            foreach (var dir in directories)
            {
                ...
                var allFiles = Directory.GetFiles(dir, "*.png").Where(f => !Path.GetFileName(f).StartsWith("merged-"))
                    .ToList();
                ...
                foreach (string file in allFiles)
                {
                    ...
                    var existingData = textureTable.TextureList.Find(x =>
                        x.TextureName.Equals(fileNameNoExt, System.StringComparison.OrdinalIgnoreCase));
        ...
        private void RefreshTileCache()
        {
            _cachedTiles.Clear();
            var allTiles =
                UnityEngine.Object.FindObjectsByType<EditMapTileComponent>(FindObjectsInactive.Exclude,
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button(_isEditingEnabled ? "Editing: ON (클릭하여 끄기)" : "Editing: OFF (클릭하여 켜기)",
                    GUILayout.Height(30)))
            {
            ...
            _focusSelectedLayer =
                EditorGUILayout.ToggleLeft("🔍 Focus Target Layer (해당 레이어 외 어둡게 표시)", _focusSelectedLayer);
            if (EditorGUI.EndChangeCheck()) UpdateTilesFocusState();

            EditorGUILayout.EndVertical();
            EditorGUILayout.Space();

            _currentMode = (EditMode)GUILayout.Toolbar((int)_currentMode,
                new string[] { "None", "Paint", "Erase", "Add", "Height", "Navi" });
            EditorGUILayout.Space();
            ...
            switch (_currentMode)
            {
                ...
                case EditMode.Add:
                    _tilePrefab =
                        (GameObject)EditorGUILayout.ObjectField("Tile Prefab", _tilePrefab, typeof(GameObject), false);
                    _samplingRoot = (EditMapSamplingComponent)EditorGUILayout.ObjectField("Sampling Root",
                        _samplingRoot,
                        typeof(EditMapSamplingComponent), true);
                    break;
                case EditMode.Height:
                    _currentSelection =
                        (SelectionMode)GUILayout.Toolbar((int)_currentSelection, new string[] { "Vertex", "Face" });
                    break;
            }
        ...
        private void DrawAtlasPaletteUI()
        {
            ...
            for (int r = 0; r < 8; r++)
            {
                for (int c = 0; c < 8; c++)
                {
                    Rect cellRect = new Rect(atlasRect.x + c * cellSize, atlasRect.y + r * cellSize, cellSize,
                        cellSize);
                    ...
                    {
                        EditorGUI.DrawRect(new Rect(cellRect.x, cellRect.y + cellSize / 2, cellSize, cellSize / 2),
        ...
        private void OnSceneGUI(SceneView sceneView)
        {
            ...
            if (e.type != EventType.Layout && e.type != EventType.Repaint)
            {
                ...
                foreach (var tile in _cachedTiles)
                {
                    ...
                    for (int i = 0; i < 13; i++)
                    {
                        Vector3 pointPos = tile.transform.TransformPoint(new Vector3(PointOffsets[i].x,
                            tile.GetPointLocalPos(i).y, PointOffsets[i].y));
        ...
        private void HandleAddMode(Ray ray, Event e)
        {
            ...
            Vector3 spawnPos =
                new Vector3(Mathf.Round(ray.GetPoint(enter).x), _targetY, Mathf.Round(ray.GetPoint(enter).z));
        ...
        private void HandleEraseMode(EditMapTileComponent tile, Event e, int controlID)
        {
            if (e.type == EventType.Repaint)
            {
                ...
                Handles.CubeHandleCap(0, tile.transform.position + new Vector3(0.5f, 0.5f, 0.5f), Quaternion.identity,
                    1.05f, EventType.Repaint);
        ...
        private void HandleHeightMode(EditMapTileComponent tile, Ray ray, Event e, int controlID)
        {
            ...
            {
                Vector3 pPos =
                    tile.transform.TransformPoint(new Vector3(PointOffsets[i].x, tile.GetPointLocalPos(i).y,
                        PointOffsets[i].y));
            ...
            if (e.type == EventType.Repaint)
            {
                ...
                {
                    Vector3 pPos = tile.transform.TransformPoint(new Vector3(PointOffsets[nearIdx].x,
                        tile.GetPointLocalPos(nearIdx).y, PointOffsets[nearIdx].y));
            ...
            if (e.type == EventType.MouseDown && e.button == 0 && !e.alt && nearIdx != -1)
            {
                ...
                foreach (int idx in (_currentSelection == SelectionMode.Vertex
                             ? new int[] { nearIdx }
        ...
        private void ExecuteOptimizeMesh()
        {
            ...
            foreach (var t in _cachedTiles)
            {
                ...
                Vector2Int gridPos = new Vector2Int(Mathf.RoundToInt(t.transform.position.x),
